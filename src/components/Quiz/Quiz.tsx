...
export const Quiz = ({ title, url, info }: Props) => {
	...
	const popup = <Popup title={title} text={info} setIsOpen={setIsOpen} />
	...
	return (
		<div className='w-full flex flex-col justify-center items-center ring-1 ring-primary-300 hover:ring-accent-yellow rounded-lg transition-all'>
			<p className='flex items-center font-poppins font-semibold text-xl mt-4 text-center min-h-[56px]'>
				{title}
			</p>
			<button
				className='px-7 py-2 bg-accent-yellow rounded-lg font-poppins font-medium text-white-50 shadow-md my-5 mx-1.5 hover:bg-primary-300 transition-colors'
				onClick={handleClick}
				type='button'
			>
				Play now
			...
			>
				...
				<p className='font-poppins text-sm font-medium text-slate-400 hover:brightness-75 transition-all'>
					Read more
				</p>
